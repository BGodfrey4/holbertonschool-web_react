...
describe("CourseList", () => {
  ...
  it("Will render 3 CourseListRow components with an empty course list", () => {
    const wrapper = shallow(<CourseList />);
    expect(wrapper.find("CourseListRow").length).toBe(3);
  });
  it('Will verify that four CourseListRows are rendered correctly with two courses passed', () => {
    const listCourses = [
